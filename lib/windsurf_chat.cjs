...
function makeRequest(port, prompt, retryCount = 0) {
  return new Promise((resolve) => {
    ...
    // req.setTimeout(480 * 60 * 1000, () => {
    //   req.destroy();
    //   console.error(`[WindsurfChat] 请求超时 (480分钟)`);
...
function formatOutput(response) {
  ...
  if (text) {
    ...
    output += `User instruction: ${cleanText}`;
  }

  if (images && images.length > 0) {
    output += '\n';
    output += '\n';
    output += `用户提供了 ${images.length} 张图片，请使用 read_file 工具读取以下图片文件：`;
    output += '\n';
  ...
  output += '\n';
  output += '\n';
  output += '重要提醒：遵循工作区规则，每次回复结束前必须调用脚本！';
...
async function main() {
  ...
  const outputFile = path.join(os.tmpdir(), `windsurf_chat_output_${uniqueId}.txt`);
  fs.writeFileSync(outputFile, output, 'utf-8');
  
  // 终端只输出简短提示
  console.log(`[WindsurfChat Output]\n请使用 read_file 工具读取完整输出：\n- ${outputFile}`);
}
